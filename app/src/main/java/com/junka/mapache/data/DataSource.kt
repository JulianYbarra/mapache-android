...
import com.junka.mapache.domain.model.*
...
class DataSource(
    ...
    val animes = localDataSource.getAnime().map { it.toDomainModel() }
    ...
    fun getAnimeById(id : Int): Flow<Anime> = localDataSource.getAnimeById(id).map { it.toDomainModel() }
}